...
from gligen.utils import _tokenize_annotated_prompt
...
def process_example_for_dd(example):
    ...
    for mask, maskprompt in masks:
        ...
        prompts.append(maskprompt)
    return "".join(promptpieces), outmasks, prompts    
...
def main(paths = [
    ...
    for path in [Path(p) for p in paths]:
        ...
        for j, inpexample in enumerate(inpexamples):
            master_prompt, masks, prompts = process_example_for_dd(inpexample)
            outimages = []
            for _ in range(numgen):
                outimages.append(process_generation(masks, -1, creg, sreg, sizereg, 1, master_prompt, *prompts)[0])
            segimg = inpexample.load_seg_image()
            outputs = [
                {"caption": master_prompt,
                "masks": masks,
                "mask_captions": prompts,
                "seg_img": segimg,
                "image": gen_image,} 
                for gen_image in outimages
            ]
